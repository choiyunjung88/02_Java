...
public class User {
	...
	// ** 오버로딩 작성 규칙 **
	// 1. 메소드 이름이 동일해야한다
	// 2. 매개변수의 자료형, 개수, 순서가 달라야한다. -- 매개변수명은 신경쓰지 않음.
	// 3. 리턴타입은 같아도 되고 달라도 된다.
	...
	public User(String userId, String userPw, String userName, int userAge, char userGender) {
		...
		// 생성자 내에서 반드시 첫째줄에 작성해야만한다 위에 다른 코드 쓰면 에러 (주석빼고)
		// 왜 사용하는 가?
	...
	// public User() {}  기본생성자가 이미 작성되어있어서 중복
	public User(String userId) {}  // 매개변수의 개수가 같은 생성자가 없다 -> 오버로딩 성립
	
	public User(int userAge) {}  // 매개변수의 개수는 같지만 타입이 다름 -> 오버로딩 성립
	
	public User(String userId, int userAge) {}  // 매개변수의 개수와 동일한 것이 있으나 하나의 타입이 다름 -> 오버로딩 성립
	
	public User(int userAge, String userId) {} // 매개변수의 개수와 타입은 같지만 순서가 다름 -> 오버로딩 성립
	
	//public User(int userAge, String userName) {} // 매개변수의 개수와 타입, 순서가 모두 같아서 오버로딩 불가
	// -> 오버로딩은 변수명을 신경쓰지 않는다.
