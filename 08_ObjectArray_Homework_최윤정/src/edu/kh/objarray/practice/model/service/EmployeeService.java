...
public class EmployeeService {
	...
	public void initEmployee() {
		for(int i = 0; i < 3; i++) {
			...
			System.out.print("급여 : ");
			int sal = sc.nextInt();
			employees[i] = new Employee(num, name, dept, job, sal);
	...
	public String allEmployeesInformation() {
		...
		for(int i = 0; i < employees.length; i++) {
			str += "\n사번 : " + employees[i].getNum()+", 이름 : "+employees[i].getName()+", 부서 : "+employees[i].getDept()+", 직급 : "+employees[i].getJob()+", 급여 : "+employees[i].getSal();
