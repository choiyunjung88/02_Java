...
public class ByteService {
	...
	public void fileByteOutput() {
		...
		try {
			...
			// -> 2byte 범주의 문자(영어, 숫자, 기본특수문자 제외)는 1byte 기반 스트림 통과 시
			...
			// 현재시간까지의 차이를 ns(nano second 단위)로 반환
			// 1ms(밀리) == 1/1000초
			// 1us(마이크로) == 1/1000ms
		...
		} finally { //try예외 발생 여부 관계없이 무조건 수행
	...
	public void fileByteInput() {
		...
		try {
			...
			while(true) {
				...
				sb.append((char)value); //char로 강제 형변환해서 글자형태로 추가
	...
	public void bufferedFileByteInput() {
		...
		// 파일 입력용 바이트 기반 스트링
				...
				try {
					...
					// 사용완료된 Stream 메모리 변환
					// 보조 스트림만 클로즈 해도 기반 스트림도 같이 클로즈 됨
					try {
